...
def plot_path_fig(model_file,data_file,circle_size=40,label_add_time=False, change_label=False, time_as_label=False,fontsize = 'x-large', use_data_labels=False,out_format="eps"):
    ...
    path_info=model['path_info']
    pars=[0]
    cell_names,cell_day,cell_labels,cell_exps,gene_names=ML.load_data(data_file,30000)
...
#             continue
        ...
        plt.text(sp_pos[0]-10, sp_pos[1]+5, 'N'+str(sp), fontdict=font)
        plt.text(sc_pos[0]-10, sc_pos[1]+5, 'N'+str(sc), fontdict=font)
        ...
        plt.text(middle[0]+x_change, middle[1]+y_change, 'edge '+str(p), fontdict=font)
    ...
    print("Total length of labs", len(labs))
    ...
    lgd=plt.legend(bbox_to_anchor=(0,1.02,1,0.2), loc="lower left",
                mode="expand", borderaxespad=0, ncol=3,
                   fancybox=True,shadow=True, fontsize = fontsize) # put outside of the plot
    for lgd_hand in lgd.legendHandles:
        lgd_hand._sizes = [circle_size]
    
    plt.axis('off')

    plt.savefig(model_file+'_cell_tree.'+out_format,dpi=300)
