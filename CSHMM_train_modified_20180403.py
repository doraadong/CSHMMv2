...
def load_data(file_name,max_gene):
    ...
    for line in lines[1:]:
        ...
        # handle cells with nan values for some genes; discard
        if len(gene_exp) != G:
            print("Different length, discarded: ",len(gene_exp), cell_name,  day, label)
        else:
...
#                 de[index,j]+=alpha
    ...
    for j in range(n_gene):
        ...
        if opt_method=="genlasso":
            ...
            g_js = cp.Variable(n_state)
            objective = cp.Minimize(0.5*cp.sum_squares((A1*g_js-Xjs)/sigma_j)+cp.pnorm(A2*g_js,1))
            constraints=[g_js>=0]
            prob = cp.Problem(objective, constraints)
            result = cp.prob.solve(solver=cp.SCS)
